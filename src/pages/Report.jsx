  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col gap-3 no-print sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Client report</h1>
          <p className="mt-1 text-sm text-zinc-500">A printable service summary.</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <button
            ...
            className="w-full rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800 sm:w-auto"
          >
            Export report
          </button>
        </div>
      ...
      <div className="mt-6 rounded-xl border border-zinc-200 bg-white p-5 sm:p-8 print:border-0 print:p-0 print:shadow-none">
        ...
        <section className="border-b border-zinc-200 py-6">
          <p className="text-sm font-medium text-zinc-500">Completion rate summary</p>
        ...
        <section className="border-b border-zinc-200 py-6">
          <h3 className="text-base font-semibold text-zinc-900">Completed &amp; evidenced jobs</h3>

          {/* Mobile: card list */}
          <div className="mt-4 space-y-3 md:hidden print:hidden">
            {evidencedJobs.map((j) => (
              <div key={j.id} className="rounded-lg border border-zinc-200 p-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-zinc-900">{j.id}</span>
                  <span className="text-xs text-zinc-400">{j.submittedTime?.replace('T', ' ') || '—'}</span>
                </div>
                <p className="mt-1 text-sm text-zinc-700">{j.taskType}</p>
                <p className="text-xs text-zinc-500">{siteName(j.siteId)}</p>
                <p className="mt-1 text-xs text-zinc-400">{operativeName(j.operativeId)}</p>
              </div>
            ))}
          ...
          <div className="mt-4 hidden overflow-x-auto md:block print:block">
            <table className="min-w-full divide-y divide-zinc-200">
              <thead>
                <tr>
                  {['Job', 'Site', 'Task', 'Operative', 'Submitted'].map((h) => (
                    <th key={h} className="whitespace-nowrap px-3 py-2 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-zinc-100">
                {evidencedJobs.map((j) => (
                  <tr key={j.id}>
                    <td className="whitespace-nowrap px-3 py-2 text-sm font-medium text-zinc-900">{j.id}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-sm text-zinc-600">{siteName(j.siteId)}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-sm text-zinc-600">{j.taskType}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-sm text-zinc-600">{operativeName(j.operativeId)}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-sm text-zinc-600">{j.submittedTime?.replace('T', ' ') || '—'}</td>
                  </tr>
                ))}
                {evidencedJobs.length === 0 && (
                  <tr>
        ...
        <section className="py-6">
          <h3 className="text-base font-semibold text-zinc-900">Exceptions</h3>
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {exceptions.map((j) => (
              ...
              >
                ...
                {j.notes && <p className="mt-2 text-sm text-zinc-600">{j.notes}</p>}
              </div>
            ))}
            {exceptions.length === 0 && (
              <p className="col-span-full py-6 text-center text-sm text-zinc-400">No exceptions in this scope.</p>
            )}
          </div>
        </section>
      </div>
